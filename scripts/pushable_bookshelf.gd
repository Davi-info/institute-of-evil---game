...
func release_acc():
	print("PushableBookshelf: Chamando release_acc()...")
	var acc_node = get_tree().current_scene.get_node_or_null("acc")
	if acc_node:
		print("PushableBookshelf: Nó 'acc' encontrado!")
		acc_node.visible = true
		acc_node.enable_acc()
		print("acc liberado da estante!")
	else:
		print("PushableBookshelf: ERRO: Nó 'acc' NÃO encontrado na cena para ser liberado.")
