...
var player_ref = null
@export var timeline_name: String = "aemario_2"  # Nome do timeline que você criou
...
func iniciar_dialogo():
	...
	Dialogic.start("res://timeline/armario_2.dtl")

func _on_body_entered(body: Node2D) -> void:
	if body.name == "Player" or body is Player:
		player_na_area = true
		player_ref = body
		print("Player detectado na lareira")

func _on_body_exited(body: Node2D) -> void:
	if body == player_ref:
		player_na_area = false
		player_ref = null
		print("Player saiu da área da lareira")
